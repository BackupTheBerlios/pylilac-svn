...
def build_words():
	...
	w["jan_mod"] = ("jan", 2, "mod", "[jan]")	#(human, somebody's, personal, of people)
	w["jan_vt"] = ("jan", 3, "vt", "begapa")	#(personify, humanize, personalize)
	...
	w["jo_kama"] = ("jo", 3, "kama", "ximumba")	#(receive, get, take, obtain)
	...
	w["kalama_vi"] = ("kalama", 3, "vi", "[kalama]")	#(make noise)
	w["kama_n"] = ("kama", 1, "n", "[kama]")	#(event, happening, chance, arrival, beginning)
	w["kama_mod"] = ("kama", 2, "mod", "[kama]")	#(coming, future)
	w["kama_vt"] = ("kama", 3, "vt", "zogapa")	#(bring about, summon)
	w["kama_vi"] = ("kama", 4, "vi", "zogipa, -apa")	#(come, become, arrive, happen, pursue actions to arrive to (a certain state), manage to, start to)
	...
	w["ken_vi"] = ("ken", 3, "vi", "dovu")	#(can, is able to, is allowed to, may, is possible)
	...
	w["kin_mod"] = ("kin", 1, "mod", "[kin]")	#(also, too, even, indeed (emphasizes the word(s) before it))
	w["kiwen_n"] = ("kiwen", 1, "n", "xami")	#(hard thing, rock, stone, metal, mineral, clay)
	w["kiwen_mod"] = ("kiwen", 2, "mod", "cayculo")	#(hard, solid, stone-like, made of stone or metal)
	w["ko_n"] = ("ko", 1, "n", "[ko]")	#(semi-solid or squishy substance, e.g. paste, powder, gum)
	w["kon_n"] = ("kon", 1, "n", "dafepi")	#(wind)
	w["kon_n_1"] = ("kon", 2, "n", "dajavi")	#(air, wind, smell, soul)
	w["kon_mod"] = ("kon", 3, "mod", "[kon]")	#(air-like, ethereal, gaseous)
	...
	w["kulupu_mod"] = ("kulupu", 3, "mod", "[kulupu]")	#(communal, shared, public, of the society)
	w["kute_mod"] = ("kute", 1, "mod", "[kute]")	#(auditory, hearing)
	...
	w["lape_mod"] = ("lape", 2, "mod", "[lape]")	#(sleeping, of sleep)
	w["laso_mod"] = ("laso", 1, "mod", "dazigo")	#(blue, blue-green)
	w["lawa_n"] = ("lawa", 1, "n", "cesi")	#(head, mind)
	w["lawa_mod"] = ("lawa", 2, "mod", "[lawa]")	#(main, leading, in charge)
	w["lawa_vt"] = ("lawa", 3, "vt", "dubusa")	#(lead, control, rule, steer)
	w["len_n"] = ("len", 1, "n", "twacapi")	#(clothing, cloth, fabric)
	w["lete_n"] = ("lete", 1, "n", "fedelo")	#(cold)
	w["lete_mod"] = ("lete", 2, "mod", "[lete]")	#(cold, uncooked)
	w["lete_vt"] = ("lete", 3, "vt", "[lete]")	#(cool down, chill)
	...
	w["linja_n"] = ("linja", 1, "n", "[linja]")	#(long, very thin, floppy thing, e.g. string, rope, hair, thread, cord, chain)
	w["lipu_n"] = ("lipu", 1, "n", "[lipu]")	#(flat and bendable thing, e.g. paper, card, ticket)
	w["loje_mod"] = ("loje", 1, "mod", "zozigo")	#(red)
	w["lon_vi"] = ("lon", 1, "vi", "[lon]")	#(be there, be present, be real/true, exist, be awake)
	w["lon_prep"] = ("lon", 2, "prep", "[lon]")	#(be (located) in/at/on)
	w["luka_n"] = ("luka", 1, "n", "twecesi")	#(hand, arm)
	w["luka_n_1"] = ("luka", 2, "n", "zicesi")	#(hand, arm)
	w["lukin_mod"] = ("lukin", 1, "mod", "[lukin]")	#(visual(ly))
	w["lukin_vt"] = ("lukin", 2, "vt", "kiva")	#(see, look at, watch, read)
	w["lukin_vt_1"] = ("lukin", 3, "vt", "kivinza")	#(see, look at, watch, read)
	w["lukin_vi"] = ("lukin", 4, "vi", "[lukin]")	#(look, watch out, pay attention)
	w["lupa_n"] = ("lupa", 1, "n", "jaytisi")	#(hole, orifice, window, door)
	w["ma_n"] = ("ma", 1, "n", "tisi")	#(land, earth, country, (outdoor) area)
	w["mama_n"] = ("mama", 1, "n", "[mama]")	#(parent, mother, father)
	w["mama_mod"] = ("mama", 2, "mod", "[mama]")	#(of the parent, parental, maternal, fatherly)
	...
	w["moli_mod"] = ("moli", 2, "mod", "[moli]")	#(dead, deadly, fatal)
	w["moli_vt"] = ("moli", 3, "vt", "kolapa")	#(kill)
	w["moli_vi"] = ("moli", 4, "vi", "kolupa")	#(die, be dead)
	w["monsi_n"] = ("monsi", 1, "n", "[monsi]")	#(back, rear end, butt, behind)
	w["monsi_mod"] = ("monsi", 2, "mod", "[monsi]")	#(back, rear)
	w["mu_interj"] = ("mu", 1, "interj", "[mu]")	#(woof! meow! moo! etc. (cute animal noise))
	w["mun_n"] = ("mun", 1, "n", "batisi")	#(moon)
	w["mun_mod"] = ("mun", 2, "mod", "[mun]")	#(lunar)
	...
	w["mute_n"] = ("mute", 1, "n", "[mute]")	#(amount, quantity)
	w["mute_mod"] = ("mute", 2, "mod", "[mute]")	#(many, very, much, several, a lot, abundant, numerous, more)
	w["mute_vt"] = ("mute", 3, "vt", "[mute]")	#(make many or much)
	w["nanpa_n"] = ("nanpa", 1, "n", "[nanpa]")	#(number)
	w["nanpa_oth"] = ("nanpa", 2, "oth", "[nanpa]")	#(-th (ordinal numbers))
	...
	w["olin_mod"] = ("olin", 2, "mod", "[olin]")	#(love)
	...
	w["pana_n"] = ("pana", 1, "n", "[pana]")	#(giving, transfer, exchange)
	...
	w["seme_oth"] = ("seme", 1, "oth", "[seme]")	#(what, which, wh- (question word))
	...
	w["sona_kama"] = ("sona", 4, "kama", "[sona]")	#(learn, study)
